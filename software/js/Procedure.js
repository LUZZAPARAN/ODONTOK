class ProcedureJs {
  ...
  //Metodo para eliminar un procedumiento
  deleteProcedure(cod_procedure) {
    swal({
      icon: "warning",
      title: "Confirmar eliminar procedimiento",
      text: "Esta segur@ de eliminar al procedimiento?",
      buttons: {
        cancel: true,
        confirm: true,
      },
      ...
      if (confirm) {
        ...
        fetch("ProcedureController/deleteProcedure", {
          method: "POST",
          body: object,
        })
          .then((resp) => resp.text())
          .then(function (data) {
            try {
              ...
              document.querySelector("#content").innerHTML = data;
              toastr.success("El procedimiento fue eliminado");
            }
          })
          .catch(function (error) {
            console.log(error);
