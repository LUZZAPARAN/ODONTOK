class PatientJs {
  ...
  deletePatient(cod_patient){
    swal({
      icon: "warning",
      title: "Confirmar eliminar Paciente",
      text: "Esta segur@ de eliminar al Paciente",
      buttons: {
        cancel: true,
        confirm: true,
      },
      ...
      if (confirm) {
        ...
        fetch("PatientController/deletePatient", {
          method: "POST",
          body: object,
        })
          .then((resp) => resp.text())
          .then(function (data) {
            try {
              ...
              document.querySelector("#content").innerHTML = data;
              toastr.success("El paciente fue eliminado");
            }
          })
          .catch(function (error) {
            console.log(error);
