<?php
    ...
    class PatientModel{
        ...
        {
            $SQL = "SELECT PAT.COD_PATIENT,PAT.NAME_PATIENT,PAT.LAST_NAME_PATIENT,PAT.NUMBER_DOCUMENT, date_part('year', age(PAT.birth_date)) AS age,
            PAT.PHONE,BLOOD.BLOOD_TYPE
            FROM ODONTOK.PATIENT PAT
            INNER JOIN ODONTOK.BLOOD_TYPE BLOOD
            ON BLOOD.COD_BLOOD_TYPE = PAT.COD_BLOOD_TYPE";
            $this->Connection->query($SQL);
        ...
        function deletePatient($cod_patient){
            $SQL = "DELETE FROM ODONTOK.PATIENT WHERE COD_PATIENT = '$cod_patient'";
            $this->Connection->query($SQL);
        }
        ...
        function searchPatientDocument($number_document){
            ...
            INNER JOIN ODONTOK.BLOOD_TYPE BLOOD
            ON BLOOD.COD_BLOOD_TYPE = PAT.COD_BLOOD_TYPE
            WHERE PAT.number_document = '$number_document'";
            $this->Connection->query($SQL);
            return $this->Connection->fetchAll();
        }
